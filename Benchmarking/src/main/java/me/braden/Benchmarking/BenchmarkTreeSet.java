...
public class BenchmarkTreeSet {
	
	@State(Scope.Benchmark)
	public static class Params {
		public Set<Integer> treeSet = new TreeSet<>();
	...
	public void benchmarkInsert(Params params) {
		for(int i = 0; i < params.numElementsToInsert; i++) {
			params.treeSet.add(i);
		}
	}
	
	@Benchmark
	public void benchmarkSort(Params params) {
		params.treeSet.stream().sorted().collect(Collectors.toSet());
	}
	
	@Benchmark
	public void benchmarkSearch(Params params) {
		params.treeSet.contains(params.searchValue);
	}
